...
@Component({
  selector: 'jhi-supplier-contacts',
...
export class SupplierContactsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.supplierContactsService
      ...
      .subscribe(
        (res: HttpResponse<ISupplierContacts[]>) => {
  ...
  trackId(index: number, item: ISupplierContacts): number {
    return item.id!;
  }

  delete(supplierContacts: ISupplierContacts): void {
    const modalRef = this.modalService.open(SupplierContactsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.supplierContacts = supplierContacts;
  ...
  protected onSuccess(data: ISupplierContacts[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/supplier-contacts'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
