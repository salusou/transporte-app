...
@Component({
  selector: 'jhi-vehicle-inspections',
...
export class VehicleInspectionsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.vehicleInspectionsService
      ...
      .subscribe(
        (res: HttpResponse<IVehicleInspections[]>) => {
  ...
  trackId(index: number, item: IVehicleInspections): number {
    return item.id!;
  }

  delete(vehicleInspections: IVehicleInspections): void {
    const modalRef = this.modalService.open(VehicleInspectionsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.vehicleInspections = vehicleInspections;
  ...
  protected onSuccess(data: IVehicleInspections[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/vehicle-inspections'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
