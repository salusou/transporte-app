...
export class CustomersGroupsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.customersGroupsService
      ...
      .subscribe(
        (res: HttpResponse<ICustomersGroups[]>) => {
  ...
  trackId(index: number, item: ICustomersGroups): number {
    return item.id!;
  }

  delete(customersGroups: ICustomersGroups): void {
    const modalRef = this.modalService.open(CustomersGroupsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.customersGroups = customersGroups;
  ...
  protected onSuccess(data: ICustomersGroups[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/customers-groups'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
